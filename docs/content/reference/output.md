...
describes what users get. Pick a format with `-o`, or let opendota choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
opendota <command> -o table   # aligned columns for reading
opendota <command> -o jsonl   # one JSON object per line, for piping
opendota <command> -o json    # a single JSON array
opendota <command> -o csv     # spreadsheet friendly
opendota <command> -o tsv     # tab-separated
opendota <command> -o url     # just the URL column
opendota <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
opendota <command> --fields id,title,url
```
...
```bash
opendota <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
opendota <command>            # a table, because this is a terminal
opendota <command> | wc -l    # JSONL, because this is a pipe
```
