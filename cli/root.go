// Package cli assembles the opendota command tree from the opendota
...
// NewApp assembles the kit application from the opendota domain. The
...
//
// To add a command, declare it in opendota/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := opendota.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(opendota.Domain{}).Register(app)
